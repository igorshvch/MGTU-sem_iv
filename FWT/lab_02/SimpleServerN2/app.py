...
from SimpleServerN2.logger import logger

app = Flask(__name__, template_folder="html", static_folder='static')

@app.before_request
def logging_request():
    logger.info(' : '.join(("REQUEST", request.method, request.url)))

@app.after_request
def logging_response(response):
    logger.info(' : '.join(("RESPONSE", request.method, request.url, response.status)))
    return response

@app.route('/service1')
def return_welcome_serv2():
    response = make_response(render_template("index.html"))
    return response

@app.route('/service1/temp')
def return_temp_serv2():
    response = make_response(render_template("temp.html"))
    return response
'''
